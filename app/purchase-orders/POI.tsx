...
const POI = ({ poItem, deletePOI }: any) => {
  const item = poItem.item;

  return (
    <tr key={poItem.id}>
      <td className="text-left px-2 py-3 whitespace-nowrap">
        <span className="text-sm text-gray-500">{poItem.id}</span>
      </td>
      ...
      <td className="text-left px-2 py-3 whitespace-nowrap">
        <span className="text-sm text-gray-500">{convertNumToPrice(poItem.quantity * item.price)}</span>
      </td>
      ...
      <td className="text-right px-2 py-3 whitespace-nowrap flex gap-2">
        <svg onClick={deletePOI} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 hover:fill-[#e11d48] cursor-pointer">
